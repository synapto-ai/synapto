...
This document serves as the master blueprint and reference for creating and executing tests across the `synapto` codebase.
...
1. **Plugin Tests**: Must be fully contained inside the respective plugin crate (e.g., `plugins/tts-google`). They must never depend on `core` or the orchestrator.
2. **Core Tests**: Scope to the `synapto` crate (`crates/core`). Used for validating core document ingestion, RAG memories, system prompts, and orchestrator state.
3. **Interface Tests**: Scope to the `synapto-interface` crate (`crates/interface`). Used solely for validating named channels, sync boundaries, or proxy structures.
