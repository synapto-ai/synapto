...
22. **Anti-Patterns: Meaningless Containers**: Avoid creating "Context" or "Settings" structs that bundle unrelated dependencies (e.g., channels, state, handles) into a single argument. This violates the Law of Total Consumption (as defined in the `synapto-ai/rfcs` repository) and creates "Data Mules." Favor explicit, atomic arguments in function signatures. This makes dependencies clear, simplifies testing, and adheres to the Single Responsibility Principle.
...
30. **Pluggable Decision Subsystem**:
    ...
    - **Provider-Independent Latency Tracking**: All decision calls dispatched through `DecisionHandle::evaluate` are automatically instrumented with `track_stats = true`, capturing call latency (`duration`), rolling average (`avg`), and peak latency (`max`) centrally in `synapto-telemetry` without coupling to specific backends.

31. **Fluent Bundle Builder Pattern**:
    Deployable bundles are composed via `Synapto::builder()` in `main.rs`. Bundle composition strictly separates configuration (`.configs()`), storage backend (`.storage()`), prompt injection (`.prompt()`), credentials (`.credentials()`), singular decision engine (`.decision()`), mandatory singular LLM engine (`.llm()`), and actor plugins (`.plugins()`) into typed, compile-time validated builder stages before calling `.run().await`. Method cardinality is strictly enforced: plural names (`.configs()`, `.credentials()`, `.plugins()`) accept tuples; singular names (`.storage()`, `.prompt()`, `.decision()`, `.llm()`) accept single types. Singletons like decision backends and LLM providers cannot be registered multiple times or misplaced in the plugins list. Mandatory core infrastructure (`configs`, `storage`, and `llm`) must be provided before terminal `.run()` becomes available.

32. **Subsystem Configuration Domain Isolation (Orthogonal Configuration Namespaces)**:
    The top-level configuration schema (`config.json`, `ConfigProvider` methods, and environment variable trees) MUST mirror the system's architectural roles 1:1. Every distinct infrastructure subsystem archetype (`cognitive`, `storage`, `credentials`, `decision`, `llm`, `plugins`) MUST reside in its own dedicated top-level configuration domain.
    - **Strict Domain Reservation:** The `"plugins"` namespace (and `load_plugin_config` / `SYNAPTO__PLUGINS__`) is strictly reserved for components implementing `synapto_interface::plugin::Plugin`.
    - **Prohibited Namespace Co-mingling:** Infrastructure providers (storage engines, ambient credential providers, decision evaluators, LLM engines, telemetry collectors) MUST NEVER reuse the `"plugins"` configuration domain or `load_plugin_config`. Placing non-plugin infrastructure settings within `"plugins"` violates Interface Segregation and Domain-Driven Design.
    - **Provider Pipeline Parity:** When a new infrastructure provider archetype is introduced to the engine, `ConfigProvider` MUST be extended with a corresponding dedicated loader method (`load_<domain>_config`) and explicit environment variable prefix (`SYNAPTO__<DOMAIN>__`).

33. **Pluggable Singleton LLM Provider**:
    The system decouples the concrete generative LLM backend from the core framework. LLM providers implement `LlmProvider` (`synapto_interface::llm::LlmProvider`) as dedicated singleton capability providers registered via the fluent bundle builder (`.llm::<L>()`). Exactly one LLM provider is mandatory per bundle and enforced at compile time via typestate markers (`NoLlm` / `WithLlm<L>`). The core initializes the configured LLM provider during bundle setup and encapsulates the low-level `RawLlmExecutor` inside an opaque `LlmExecutor` handle supplied to plugins and cognitive loops. Dedicated LLM configuration lives under the isolated `"llm"` configuration namespace.
...
- **LLMSafe Boundaries**: `LLMSafe` structs (data models specifically formatted for the LLM context) MUST NOT be part of the cross-boundary interface (`synapto-interface`). They must remain internal to the core crate. Enforce this by restricting their visibility to `pub(crate)` instead of `pub`, and mapping from generic interface types (e.g., `synapto_interface::core::Document`) to `LLMSafe` types via `From`/`Into` traits within the core.
- **Interface Purity**: The `synapto-interface` crate MUST contain ONLY pure generic interface traits and universally shared DTOs (Data Transfer Objects). Never blindly move a highly specialized or domain-specific struct (e.g., `State`, `InteractionMemory`, `PluginInstruction`) from `synapto` to `synapto-interface` just to satisfy a dependency. If a plugin needs to communicate with the core using a core concept, define a simplified, generic abstraction in the interface, and implement `From`/`Into` translation boundaries inside `synapto`.
...
To keep the `synapto` agnostic to external environments, we isolate static prompts and configuration loading mechanisms into dedicated, interchangeable modules provided during bootstrap.
...
- **Config Providers**: Implementation of the `ConfigProvider` trait live in the `config-providers/` directory (e.g., `config-providers/memory`, `config-providers/file`). They manage how settings (API keys, models, system flags) are retrieved and parsed.
  - **Environment Variable Overrides & Case-Sensitivity**: Providers that read environment variables (`Env`, `DotEnv`) map double-underscore (`__`) separated variables into nested JSON structures. Operators must strictly observe code casing because environment variable lookup is case-sensitive on Unix systems and Serde models enforce `#[serde(deny_unknown_fields)]`. Subsystem prefixes are uppercase (`SYNAPTO__`, `PLUGINS__`, `STORAGE__`, `CREDENTIALS__`, `DECISION__`), whereas crate names are `snake_case`, provider/plugin struct names are `PascalCase`, and struct fields are `snake_case` (e.g., `SYNAPTO__CREDENTIALS__synapto_credentials_typesafe__TypeSafeCredentials__api_key` or `SYNAPTO__DECISION__synapto_decision_typesafe__TypeSafeDecision__model`).
  - **Reserved Top-Level Configuration Domains**: The root configuration schema enforces strictly partitioned, orthogonal domains:
    - `cognitive`: LLM reasoning models, effort parameters, and sampling settings.
    - `storage`: Backend database engines and storage pools (e.g., Firestore, SurrealDB, LocalStorage).
    - `credentials`: Cloud identity resolvers and authentication keys.
    - `decision`: Dedicated decision providers and System One judgment evaluators.
    - `plugins`: Background actor loops, sensors, peripheral controllers, and observers.
    Overloading or nesting one subsystem's configuration inside another is strictly forbidden.
