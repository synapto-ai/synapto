...
32. **Subsystem Configuration Domain Isolation (Orthogonal Configuration Namespaces)**:
    ...
    - **Prohibited Namespace Co-mingling:** Infrastructure providers (storage engines, ambient credential providers, decision evaluators, telemetry collectors) MUST NEVER reuse the `"plugins"` configuration domain or `load_plugin_config`. Placing non-plugin infrastructure settings within `"plugins"` violates Interface Segregation and Domain-Driven Design.
