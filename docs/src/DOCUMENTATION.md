...
- **What it MUST contain:**
  ...
  - Minimal registration and async-runtime bootstrapping boilerplate (`Synapto::builder().configs(...).storage(...).plugins(...).run().await`).
- **What it MUST NOT contain:**
...
- **Crate Documentation Boundaries**: Documentation inside any crate directory (such as `crates/*`) must not mention anything outside its own boundaries. Internal crate documentation and doc comments must describe only the types, traits, interfaces, and behaviors provided by that crate. Do not reference concrete implementations, external downstream crates, or external integrations.
