...
- **LLMSafe Boundaries**: `LLMSafe` structs (data models specifically formatted for the LLM context) MUST NOT be part of the cross-boundary interface (`synapto-interface`). They must remain internal to the core crate. Enforce this by restricting their visibility to `pub(crate)` instead of `pub`, and mapping from generic interface types (e.g., `synapto_interface::types::Document`) to `LLMSafe` types via `From`/`Into` traits within the core.
