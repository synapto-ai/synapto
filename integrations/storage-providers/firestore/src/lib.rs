...
use firestore::{
    FirestoreDb, FirestoreFindNearestDistanceMeasure, FirestoreQueryDirection, FirestoreQueryOrder,
    FirestoreVector,
};
...
impl RecordStore for FirestoreStorage {
        ...
        T: serde::de::DeserializeOwned + Send + Sync + 'static,
    {
        ...
        let direction = match order {
            synapto_interface::storage::SortOrder::Descending => {
    ...
    async fn trim_records_before(&self, collection: &str, cutoff_key: &str) -> Result<(), String> {
        ...
        let stream = self
            ...
            .filter(|q| {
                q.for_all([q
                    .field(firestore::path!(firestore::FirestoreDocument::name))
                    .less_than(cutoff_key)])
            })
            .stream_query_with_metadata()
            .await
            .map_err(|e| e.to_string())?;
        ...
        while let Some(item_res) = stream.next().await {
            if let Ok(item) = item_res
                && let Some(doc) = item.document
            {
                let id = doc.name.split('/').next_back().unwrap_or_default();
                if let Err(e) = self.delete_record(collection, id).await {
                    tracing::error!(
                        "Failed to delete record '{id}' during trim in collection '{collection}': {e}"
                    );
                }
            }
        }
