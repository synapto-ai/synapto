...
async fn test_google_tts_live_synthesis_xml_escaping() {
    ...
    impl synapto_interface::llm::RawLlmExecutor for DummyLlmExecutor {
    ...
    let storage_handle = synapto_interface::storage::StorageHandle::new(std::sync::Arc::new(
        DummyStorageConfigResolver,
    ));
