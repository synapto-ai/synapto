...
impl ChatPlugin for MumblePlugin {
    async fn start(
        ...
        self.__channels
            ...
            .map_err(|_| "Chat channels are already initialized".to_string())?;
...
impl AudioInputPlugin for MumblePlugin {
    async fn start(&self, tx: mpsc::Sender<PeerInputAudio>) -> Result<(), String> {
        self.__channels
            .audio_input
            .set(AudioInputChannels {
                peer_input_audio_tx: tx,
            })
            .map_err(|_| "Audio input channels are already initialized".to_string())?;
        self.__generated_try_start_barrier().await
    }
}
...
impl AudioOutputPlugin for MumblePlugin {
    async fn start(&self, rx: mpsc::Receiver<CognitiveOutputAudio>) -> Result<(), String> {
        self.__channels
            .audio_output
            .set(AudioOutputChannels {
                cognitive_output_audio_rx: StdMutex::new(Some(rx)),
            })
            .map_err(|_| "Audio output channels are already initialized".to_string())?;
        self.__generated_try_start_barrier().await
    }
}
...
impl MumblePlugin {
    ...
    async fn start_internal(&self) -> Result<(), String> {
        let chat_channels = self
            .__channels
            .chat
            .get()
            .ok_or_else(|| "Chat channels are missing".to_string())?;
        let audio_input_channels = self
            .__channels
            .audio_input
            .get()
            .ok_or_else(|| "Audio input channels are missing".to_string())?;
        let audio_output_channels = self
            .__channels
            .audio_output
            .get()
            .ok_or_else(|| "Audio output channels are missing".to_string())?;
        ...
        let mut cognitive_output_text_rx = chat_channels
            ...
            .take()
            .ok_or_else(|| "Missing cognitive_output_text_rx channel".to_string())?;
        ...
        let mut cognitive_output_audio_rx = audio_output_channels
            ...
            .map_err(|e| format!("Lock poisoned: {e}"))?
            .take()
            .ok_or_else(|| "Missing cognitive_output_audio_rx channel".to_string())?;
